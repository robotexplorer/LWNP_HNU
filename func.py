...
def TV_minimization_ori(x, y, _lambda, n_iter_max):
    dt = 0.24
    N = x.shape
    idx = torch.arange(1, N[0]+1)
    idx[-1] = N[0]-1
...
def TV_minimization_v2(x, y, _lambda, n_iter_max):
    dt = 0.24
    N = x.shape
    idx = torch.arange(1, N[0]+1)
    idx[-1] = N[0]-1
    ...
    for i in range(n_iter_max):
        ...
        z4 = 0.4*(z - z[iux,:,:])
        z5 = 0.23*(z[idx,:,:] - z[:,ir,:])
        denom_2d = 1 + dt*torch.sqrt(torch.sum(z1**2 + z2**2 + z3**2 + z4**2 + z5**2, 2))
        denom_3d = torch.unsqueeze(denom_2d, 2).repeat(1, 1, N[2])
        p1 = (p1+dt*z1)/denom_3d
        p2 = (p2+dt*z2)/denom_3d
        p3 = (p3+dt*z3)/denom_3d
        p4 = (p4+dt*z4)/denom_3d
        p5 = (p5+dt*z5)/denom_3d
        divp = p1-p1[:,il,:] + p2-p2[iux,:,:] + p3[:,ir,:]-p3 + p4[idx,:,:]-p4 + p5[:,il,:]-p5[iux,:,:]
    u = x - divp/_lambda
    return u
...
import os
def saveMats(inputs,name,dirs):
    if not os.path.exists(dirs):
        os.makedirs(dirs)
    sio.savemat(dirs+'ReconMat'+str(name)+'.mat', {'img_hs':inputs})
