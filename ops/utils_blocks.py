...
class block_module():
    ...
    def _make_blocks(self, image, return_padded=False):
        ...
        image_mirror_padded = F.pad(image, pad, mode='reflect')#pad(63,63,63,63)
        # print('>> image_mirror_padded.shape = ',image_mirror_padded.shape)#(1,31,382,382)
        ...
        if params['centered_pad']:
            ...
            pad_ =(0, pad_w, 0, pad_h)#(0,2,0,2)
        pad = tuple([x+y for x,y in zip(pad,pad_)])
        # print('>> pad ',pad)#pad(63,65,63,65)
        self.pad = pad

        image_mirror_padded_even = F.pad(image, pad, mode='reflect')  # add half kernel cause block edges are dump

        # print('>> image_mirror_padded_even.shape = ',image_mirror_padded_even.shape)#torch.Size([1, 31, 384, 384])
        ...
        # print('>> batch_blocks.shape',batch_blocks.shape)#torch.Size([1, 126976, 81])
        ...
        # print('>> batch_blocks.shape',batch_blocks.shape)# torch.Size([81, 31, 64, 64])
        return batch_blocks
